        ...
        fluidRow(
          ...
          box(
            ...
            fluidRow(
              ...
              column(width = 12, numericInput(inputId = ns("cv_n"), label = "Cross Validation Fold", value = 5, min = 1)),
              column(width = 12, selectInput(inputId = ns("msr_type"), label = "Measure", choices = c("roc", "prc", "predict"), selected = "roc"))
            ),
            download_plot_UI("mlr_rsmp")
          )
        )
      ),
      tabPanel(
